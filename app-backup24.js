...
// Sprint 30: 成長・AIコーチ・提出期限ページの見出しにも、同じエンブレムを一度だけ配置する(表示のみ)。
(function insertChampionPageEmblems(){
 document.querySelectorAll('.growth-hero h1,.report-hero h1,.assignment-hero h1').forEach(h1=>{
  if(h1.querySelector('.champion-emblem'))return;
  h1.insertAdjacentHTML('beforebegin','<span class="champion-emblem" aria-hidden="true"></span>');
 });
})();
themeToggle.onclick=()=>{
