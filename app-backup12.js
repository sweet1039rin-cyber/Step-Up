...
function saveAssignmentItems(id,items){
 const states={};
 items.forEach(item=>{states[item.id]={status:item.status,progress:item.progress,remaining:item.remaining,current:item.current,total:item.total,deadline:item.deadline,done:item.done}});
...
const ProgressEngine={
 getAll(id){
  ...
  const canonical=assignmentItems(id).map(it=>({
   ...
   deadline:it.deadline||null,isCustom:false
  }));
  const customs=getCustomAssignments(id).map(c=>({
   ...
   deadline:c.deadline||null,scope:c.scope||'',isCustom:true
  }));
...
function renderGrowth(){
 ...
 if(current==='iori'){
  const physicsDone=ProgressEngine.getAll('iori').filter(a=>a.name.startsWith('家庭教師 物理')&&a.done).length;
...
// Sprint 16: 進捗更新モーダル（prompt()の代わりにアプリ内モーダルを使用）
const progressModal=document.querySelector('#progressModal');
const progressModalValue=document.querySelector('#progressModalValue');
const progressModalTotal=document.querySelector('#progressModalTotal');
const progressModalConfirm=document.querySelector('#progressModalConfirm');
const progressModalCancel=document.querySelector('#progressModalCancel');
const progressModalMinus=document.querySelector('#progressModalMinus');
const progressModalPlus=document.querySelector('#progressModalPlus');
const progressModalSuccess=document.querySelector('#progressModalSuccess');
let progressModalContext=null;
let progressModalBusy=false;
function clampProgressValue(value,total){
 const max=Number.isFinite(Number(total))?Number(total):9999;
 return Math.max(0,Math.min(max,Math.round(Number(value)||0)));
}
function openProgressModal(assignment){
 if(!progressModal)return;
 progressModalContext={id:assignment.id,total:assignment.total};
 progressModalValue.value=assignment.current;
 progressModalTotal.textContent=`全体：${assignment.total}`;
 progressModalSuccess.classList.add('hidden');
 progressModalConfirm.disabled=false;
 progressModalBusy=false;
 progressModal.classList.remove('hidden');
}
function closeProgressModal(){
 progressModal?.classList.add('hidden');
 progressModalContext=null;
}
progressModalMinus?.addEventListener('click',()=>{
 if(!progressModalContext)return;
 progressModalValue.value=clampProgressValue(Number(progressModalValue.value||0)-1,progressModalContext.total);
});
progressModalPlus?.addEventListener('click',()=>{
 if(!progressModalContext)return;
 progressModalValue.value=clampProgressValue(Number(progressModalValue.value||0)+1,progressModalContext.total);
});
progressModalCancel?.addEventListener('click',()=>{if(!progressModalBusy)closeProgressModal()});
progressModalConfirm?.addEventListener('click',()=>{
 if(progressModalBusy||!progressModalContext)return;
 progressModalBusy=true;
 progressModalConfirm.disabled=true;
 const {id,total}=progressModalContext;
 const value=clampProgressValue(progressModalValue.value,total);
 const all=getAssignments();
 const a=all.find(x=>String(x.id)===String(id));
 if(a){
  a.current=value;
  saveAssignments(all);
  syncLearningProgress(current,a.name,a.current,a.total);
  renderAssignments();
 }
 progressModalSuccess.classList.remove('hidden');
 setTimeout(()=>{
  progressModalBusy=false;
  closeProgressModal();
 },1000);
...
function renderAssignments(){
 if(current==='sakuya'){renderSakuyaSummerAssignments();return}
 assignmentPerson.textContent=data[current].name+' / 夏休み課題';
 const list=getAssignments().sort((a,b)=>{
  const doneA=a.current>=a.total,doneB=b.current>=b.total;
 ...
 assignmentSummary.innerHTML=`<div class="assignment-summary-line">登録 ${list.length}件　｜　期限間近 ${urgent}件　｜　残り ${remaining}</div>`;
 ...
 document.querySelectorAll('[data-assignment-progress]').forEach(b=>b.onclick=()=>{const all=getAssignments(),a=all.find(x=>String(x.id)===b.dataset.assignmentProgress);if(a)openProgressModal(a)});
...
function openAssignments(){assignmentForm.classList.add('hidden');if(addAssignmentBtn)addAssignmentBtn.classList.toggle('hidden',current==='sakuya');document.querySelector('#testInfoDetails')?.classList.toggle('hidden',current==='sakuya');renderAssignments();show(assignmentsScreen)}
...
// Sprint 13: 朔埜の夏休み課題（進捗はパーセントではなく自由記述＋完了チェックで管理）
...
function sakuyaSummerKey(){return 'stepup-summer-assignments-sakuya'}
function getSakuyaSummerAssignments(){
 return ProgressEngine.getAll('sakuya').map(it=>({
  id:it.id,subject:it.subject,name:it.title,
  scope:it.isCustom?(it.scope||''):[it.progress,it.remaining].filter(x=>x&&x!=='なし').join(' / '),
  deadline:it.deadline||'',progress:it.progress||'',done:it.done
 }));
}
function saveSakuyaSummerAssignments(list){
 list.forEach(item=>{
  const done=!!item.done;
  ProgressEngine.updateItem('sakuya',item.id,{
   progress:item.progress,
   status:done?'completed':(item.progress&&item.progress.trim()?'in-progress':'not-started'),
   done
  });
 });
}
function renderSakuyaSummerAssignments(){
 assignmentPerson.textContent='朔埜の課題';
 const list=getSakuyaSummerAssignments();
 const total=list.length,done=list.filter(a=>a.done).length,remaining=total-done;
 assignmentSummary.innerHTML=`<div class="assignment-summary-line">登録 ${total}件　｜　完了 ${done}件　｜　残り ${remaining}</div>`;
 const bySubject={};const order=[];
 list.forEach(a=>{if(!bySubject[a.subject]){bySubject[a.subject]=[];order.push(a.subject)}bySubject[a.subject].push(a)});
 assignmentList.innerHTML=order.map(subject=>{
  const items=bySubject[subject].map(a=>{
   const progressNote=a.progress&&a.progress.trim()?`ここまで：${escapeHtml(a.progress)}`:'まだ記録なし';
   return `<article class="assignment-item sakuya-summer-item ${a.done?'done':''}">
    <div class="assignment-top"><div><small>${escapeHtml(a.subject)}${a.scope?'・'+escapeHtml(a.scope):''}</small><h2>${escapeHtml(a.name)}</h2></div><b>${a.deadline?'提出：'+escapeHtml(a.deadline):'提出日指定なし'}</b></div>
    <div class="sakuya-progress-note">${progressNote}</div>
    <label class="sakuya-progress-input">どこまで進んだか
     <textarea data-sakuya-progress="${a.id}" placeholder="例：P45まで終わった">${escapeHtml(a.progress||'')}</textarea>
    </label>
    <label class="sakuya-done-check"><input type="checkbox" data-sakuya-done="${a.id}" ${a.done?'checked':''}> 完了した</label>
   </article>`;
  }).join('');
  return `<div class="sakuya-subject-group"><h3>【${escapeHtml(subject)}】</h3>${items}</div>`;
 }).join('');
 document.querySelectorAll('[data-sakuya-progress]').forEach(t=>{
  t.addEventListener('change',()=>{
   const all=getSakuyaSummerAssignments();
   const item=all.find(x=>String(x.id)===t.dataset.sakuyaProgress);
   if(!item)return;
   item.progress=t.value;
   saveSakuyaSummerAssignments(all);
   renderSakuyaSummerAssignments();
  });
 });
 document.querySelectorAll('[data-sakuya-done]').forEach(cb=>{
  cb.addEventListener('change',()=>{
   const all=getSakuyaSummerAssignments();
   const item=all.find(x=>String(x.id)===cb.dataset.sakuyaDone);
   if(!item)return;
...
function applyAssignmentReport(id,currentText){
 ...
 // customItems（家庭教師 物理などcanonicalに無い課題）も同じ報告文から更新する
 const customs=getCustomAssignments(id);
 let customsChanged=false;
 customs.forEach(item=>{
  const title=normalizeAssignmentText(item.title);
  if(title&&normalizedText.includes(title)&&/(完了|終わ|進め|やった|できた|ページ|p\d+)/i.test(currentText)){
   item.status='completed';item.done=true;item.progress='報告内容から完了を確認';item.remaining='なし';
   if(item.total!=null)item.current=item.total;
   customsChanged=true;
   if(!completedItems.some(completed=>completed.id===item.id))completedItems.push(item);
  }
 });
 if(customsChanged)saveCustomAssignments(id,customs);
