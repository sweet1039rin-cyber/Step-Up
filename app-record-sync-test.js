...
function key(){return 'stepup-v4-'+PLAN_DATE+'-'+current}
 ...
 async function savePlanToFirebase(saved){const f=window.stepUpFirebase,ref=firebaseCheckRef();if(!f||!ref)return;try{await f.setDoc(ref,{customTasksJson:JSON.stringify(Array.isArray(saved.customTasks)?saved.customTasks:[]),planUpdatedAt:Date.now()},{merge:true})}catch(e){console.error("plan sync save failed",e);alert("計画同期保存エラー："+(e.code||e.message))}}
 async function saveDailyReportToFirebase(report){const f=window.stepUpFirebase,ref=firebaseCheckRef();if(!f||!ref)return;try{await f.setDoc(ref,{dailyReportJson:JSON.stringify(report),dailyReportUpdatedAt:Date.now()},{merge:true});alert("学習記録同期保存成功")}catch(e){console.error("daily report sync save failed",e);alert("学習記録同期保存エラー："+(e.code||e.message))}}
 ...
 if(typeof remote.dailyReportJson==="string"){try{const report=JSON.parse(remote.dailyReportJson);if(report&&report.date){localStorage.setItem(dailyReportKey(report.date),JSON.stringify(report));if(report.date===dateKey(new Date())&&key()===nextKey)loadDailyReportCard()}}catch(e){console.error("daily report sync parse failed",e)}}
      local.checks=remote.checks||{};
 if(typeof remote.customTasksJson==="string"){try{local.customTasks=JSON.parse(remote.customTasksJson)}catch(e){console.error("plan sync parse failed",e)}}
...
}).join('');stepMessage.textContent=saved.step||'今日の記録はまだありません。「今日の記録を保存する」を押すと、ここに表示されます。';bindChecks();update();setTimeout(()=>{try{startCheckSync()}catch(e){alert("同期開始エラー: "+(e.code||e.message))}},1500);renderPersonalCoach();renderHomeDeadlineCard();loadDailyReportCard();renderSakuyaTestRulesCard();renderDokosutaHistory()}
...
function saveDailyReportCard(){
 ...
 saveDailyReportToFirebase({date,done,stepUp});
