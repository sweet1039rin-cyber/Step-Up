...
function getSakuyaSummerAssignments(){
 try{
  const saved=JSON.parse(localStorage.getItem(sakuyaSummerKey())||'null');
  if(Array.isArray(saved)&&saved.length)return saved;
 }catch(e){console.error('朔埜の夏休み課題データの読み込みに失敗しました',e)}
 return sakuyaSummerDefaults.map(x=>({...x}));
}
function saveSakuyaSummerAssignments(list){
 try{localStorage.setItem(sakuyaSummerKey(),JSON.stringify(list))}
 catch(e){console.error('朔埜の夏休み課題データの保存に失敗しました',e)}
}
