...
function openAssignments(){assignmentForm.classList.add('hidden');if(addAssignmentBtn)addAssignmentBtn.classList.toggle('hidden',current==='sakuya');renderAssignments();show(assignmentsScreen)}
...
function renderSakuyaSummerAssignments(){
 ...
 assignmentSummary.innerHTML=`<article><b>${total}</b><span>登録課題</span></article><article><b>${done}</b><span>完了</span></article><article><b>${remaining}</b><span>残り</span></article>`;
