...
function assignmentStepText(id=current,completedItems=[],additionalProgress=''){
 // Sprint 21: 今回チェックされた項目(completedItems)だけを使う。
 // 現在のミッションや次の予定、未チェックの課題を勝手に混ぜない。
 const names=completedItems.map(item=>item.title);
 if(names.length===1)return `${names[0]}に取り組めたことが、今日の大切な一歩です。`;
 if(names.length>1)return `${names[0]}をはじめ${names.length}件に取り組めたことが、今日の大切な一歩です。`;
